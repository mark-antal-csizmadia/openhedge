...
async def get_events(client: httpx.AsyncClient, limiter: AsyncLimiter, params: GetEventsRequest) -> GetEventsResponse:
...
async def get_series(client: httpx.AsyncClient, limiter: AsyncLimiter, params: GetSeriesRequest) -> GetSeriesResponse:
    """Get series from the Kalshi REST API."""
    async with limiter:
        response = await client.get(url=f"{SERIES_URL}/{params.series_ticker}")
    response.raise_for_status()
    return GetSeriesResponse.model_validate(response.json())
...
) -> AsyncIterator[GetEventsResponse]:
